...
func (g *SQLGenerator) formatDefaultValue(colType, defaultValue string) string {
	...
	if strings.Contains(lower, "()") || // Functions like gen_cuid(), now(), uuid_generate_v4()
		lower == "true" || lower == "false" || // Boolean values
		strings.HasPrefix(lower, "nextval(") || // Sequences
		strings.HasPrefix(lower, "'{") || // JSON/JSONB literals that are already quoted
		strings.HasPrefix(lower, "'") || // Already quoted strings
