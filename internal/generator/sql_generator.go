...
func (g *SQLGenerator) GenerateCreateTable(table SchemaTable) string {
	...
	// Other constraints from table definition
	for _, constraint := range table.Constraints {
		...
		case "FOREIGN KEY":
			// Foreign keys are handled at column level for simple cases
			continue
		}
	}
...
func (g *SQLGenerator) generateColumnDDL(col SchemaColumn) string {
	...
	// UNIQUE (if not part of a composite unique constraint)
...
func (g *SQLGenerator) isImplicitIndex(idx SchemaIndex, table SchemaTable) bool {
	// Primary key indexes are implicit
	if idx.IsPrimary {
		return true
	}
	
	// Unique constraint indexes might be implicit
	if idx.IsUnique && len(idx.Columns) == 1 {
		// Check if there's a unique column constraint
		for _, col := range table.Columns {
...
func (g *SQLGenerator) GenerateSchema(schema *DatabaseSchema) string {
	...
	sql.WriteString("-- Enable UUID extension\n")
	sql.WriteString("CREATE EXTENSION IF NOT EXISTS \"uuid-ossp\";\n\n")
	
	// Add CUID generation function if any table uses CUIDs
	if g.schemaUsesCUIDs(schema) {
		...
		sql.WriteString("    -- CUID components\n")
		...
		sql.WriteString("    v_result TEXT := 'c'; -- CUID prefix\n")
		sql.WriteString("BEGIN\n")
		sql.WriteString("    -- Timestamp in milliseconds since epoch\n")
		sql.WriteString("    v_timestamp := FLOOR(EXTRACT(EPOCH FROM clock_timestamp()) * 1000);\n")
		sql.WriteString("    \n")
		sql.WriteString("    -- Global counter (using advisory lock for safety)\n")
		sql.WriteString("    v_counter := nextval('cuid_counter_seq');\n")
		sql.WriteString("    \n")
		sql.WriteString("    -- Machine fingerprint (using database system identifier)\n")
		sql.WriteString("    v_fingerprint := encode(digest(current_database() || inet_server_addr()::TEXT, 'sha256'), 'hex');\n")
		sql.WriteString("    \n")
		sql.WriteString("    -- Add timestamp (base36 encoded, 8 chars)\n")
		sql.WriteString("    v_result := v_result || lpad(to_base36(v_timestamp), 8, '0');\n")
		sql.WriteString("    \n")
		sql.WriteString("    -- Add counter (base36 encoded, 4 chars)\n")
		sql.WriteString("    v_result := v_result || lpad(to_base36(v_counter % 1679616), 4, '0'); -- 36^4\n")
		sql.WriteString("    \n")
		sql.WriteString("    -- Add fingerprint (4 chars from hash)\n")
		sql.WriteString("    v_result := v_result || substr(v_fingerprint, 1, 4);\n")
		sql.WriteString("    \n")
		sql.WriteString("    -- Add random component (8 chars)\n")
	...
	// Generate tables in dependency order (simple version - alphabetical for now)
...
func (g *SQLGenerator) schemaUsesCUIDs(schema *DatabaseSchema) bool {
	for _, table := range schema.Tables {
		for _, col := range table.Columns {
			...
			// Check default values for CUID generation functions
